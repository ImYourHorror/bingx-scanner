...
ДАННЫЕ / ИСТОЧНИКИ (всё проверено против живых API 2026-06-30):
  ...
  • Андерлайн (колонка vs TW% + база графика): TradingView screener (тот же, что и клоуз), delayed ~15м.
        vsTW% = (lastPrice_ws − tw) / tw; нет US/ADR → «—». Pyth оставлен за флагом USE_PYTH (по умолч. выкл.).
...
# Андерлайн (колонка «vs TW%» + базовая линия графика) = TradingView screener (delayed ~15м).
    ...
    for it in inst:
        ...
        if pm:
            try:
                ...
                funding = None
        # Андерлайн UI = TradingView (delayed ~15м): колонка «vs TW%» + базовая линия графика.
        tw_val = None
        tp = tw.get(base) if tw else None
        if tp and tp[0]:
            tw_px = tp[0]
            if live is not None:
                tw_val = (live - tw_px) / tw_px * 100
            if SERIES is not None:
                SERIES.add_base(sym, tw_px, time.time())
        ...
        row = {
            ...
            "premium": premium, "tw": tw_val, "basis": basis, "funding": funding, "oi": oi,
...
<div class="sub">live = WebSocket lastPrice · гэп = (live − close)/close · vs TW = базис к цене TradingView (задержка 15м) · клик по строке → график · обновлено <span id="upd">—</span></div>
<div class="bar">
...
<th data-key="tw" title="цена TradingView с задержкой ~15 мин (тот же screener, что и клоуз)">vs TW% <span class="mut" style="font-weight:400">(15м)</span><span class="ar"></span></th>
...
<b>vs TW%</b> = базис перпа к цене TradingView (задержка ~15м) · гэп «—» = нет клоуза/данные сомнительны (наведи) · OI убран из таблицы, пишется в лог
</div>
</div>
...
<p><b>Гэп% = (BingX − close) / close</b> — насколько перп ушёл от вчерашнего закрытия. «—» если нет надёжного клоуза, либо гэп &gt;25% — тогда строка «данные сомнительны» (наведи на «—»: причина + сырой %). Лучше прочерк, чем мусор.</p>
<p><b>vs TW% = (BingX − TW) / TW</b> — кросс-чек перпа против цены TradingView (тот же screener, что и клоуз). <b>TW отстаёт ~15 мин</b>, поэтому на резком движении спред отражает ЛАГ TradingView, а не реальную дислокацию перпа. Нет TW (иностранные без US/ADR) → «—». (Pyth-андерлайн отключён флагом <code>USE_PYTH</code>, код на месте.)</p>
...
function defcmp(a,b){const sa=a.gap!=null?Math.abs(a.gap):(a.tw!=null?Math.abs(a.tw):-1);
  const sb=b.gap!=null?Math.abs(b.gap):(b.tw!=null?Math.abs(b.tw):-1);return sb-sa;}
...
function makeRow(x){const tr=document.createElement('tr');
  ...
  tr.innerHTML=`<td class="l"><span class="dot ${x.fam}"></span>${x.ticker}${x.is_regime?' ★':''}</td>`+
    `<td></td><td class="mut"></td><td></td><td></td><td class="mut"></td>`;
  return {tr};}
function updateRow(o,x){const ch=o.tr.children;
  ...
  ch[4].textContent=sgn(x.tw,3);ch[4].className=cls(x.tw);
  ch[5].textContent=(x.funding&&x.funding.rate!=null)?sgn(x.funding.rate,4)+'%/'+(x.funding.ih||'?')+'h':'—';}
...
function ensureChartRow(){if(!CHARTROW){CHARTROW=document.createElement('tr');CHARTROW.className='chartrow';
    CHARTROW.innerHTML='<td colspan="6"></td>';}return CHARTROW;}
